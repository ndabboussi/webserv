...
// vec2
// BigInt
// string de nb en entree, faut la stocker et la print peut importe ce qu'on te donne'
...
std::string buildResponseContent(HttpResponse &resp, 
                                ...
                                const std::string &contentType = "text/html")
{
    ...
    response << "Connection: " << setConnection(req);

    // Step 3: Optional body (depends on status code)
    if (resp.code != 204 && resp.code != 304 && !body.empty())
    {
...
resp.code = 200;
setStatusLine(resp);
std::string responseStr = buildResponseContent(resp, request, body, getContentType(request.path));
...
std::string buildResponseContent(HttpResponse &resp, const HttpRequest &req, const std::string &body, const std::string &contentType, bool includeBody)
{
	std::ostringstream response;
	...
	// Step 3: Decide if we must send a body
	bool sendBody = true;

	if (resp.code >= 100 && resp.code < 200) // Informational
		sendBody = false;
	else if (resp.code == 204 || resp.code == 304) // No body allowed
		sendBody = false;

	// Step 4: Add headers & body if allowed
